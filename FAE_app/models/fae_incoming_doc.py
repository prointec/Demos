...
class XFaeIncomingDoc(models.Model):
    ...
    def unlink(self):
        for rec in self:
            if rec.code_accept:
                raiseValidationError('El documento: %s ya tiene aceptación' % (rec.issuer_sequence))
            elif rec.invoice_id:
                raiseValidationError('El documento: %s ya está asociado con documento ingresado en Proveedor' % (rec.issuer_sequence))
        return super(XFaeIncomingDoc, self).unlink()

    @api.onchange('code_accept')
    def _onchange_code_accept(self):
        if self.code_accept in ('A','P','R'):
