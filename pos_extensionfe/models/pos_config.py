...
class POSConfigInherit(models.Model):
    ...
    def _compute_current_session(self):
        for pos_config in self:
            opened_sessions = pos_config.session_ids.filtered(lambda s:  not s.x_employee_id and not s.state == 'closed')
            session = pos_config.session_ids.filtered(lambda s: not s.x_employee_id and not s.state == 'closed' and not s.rescue).sorted(lambda r: r.id, reverse=True)
