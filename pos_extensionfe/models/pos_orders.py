...
class PosOrderInherit(models.Model):
    ...
    x_move_type = fields.Selection(string="Move Type",
    ...
    def action_recrea_documento(self):
        ...
        if not sale_session:
            raise ValidationError('Debe seleccionar un Cajero antes de poder recrear un documento')
    ...
    def write(self, vals):
        ...
        for order in self:
            ...
            if order.x_document_type == 'FE' and order.partner_id and not (order.partner_id.x_identification_type_id and order.partner_id.vat):
                raise ValidationError('Para Emitir la facturación Electrónica el cliente debe tener definido el tipo de identificáción y el número')  
            ...
            if order.company_id.x_fae_mode in ('api-stag', 'api-prod'):
                ...
                if order.currency_id.name != order.company_id.currency_id.name and (not order.currency_id.rate_ids or not (len(order.currency_id.rate_ids) > 0)):
                ...
                if order.state == 'draft':
                    ...
                    for line in order.lines:
                        if line.product_id and not (line.product_id.x_cabys_code_id and line.product_id.x_cabys_code_id.code):
    ...
    def genera_consecutivo_dgt(self):
        if not self.x_document_type or self.x_sequence or self.x_electronic_code50:   
            return
    ...
    def _prepare_invoice_vals(self):
        ...
        vals.update({'x_document_type': self.x_document_type,
    ...
    def action_pos_order_invoice(self):
        self.ensure_one()
        if self.state != 'draft':
            raise UserError('La Orden de POS debe estar en estado DRAFT (pendiente)')
        elif not self.partner_id:
            raise UserError('Para poner a crédito la Factura debe tener un código de cliente')
    ...
    def generate_xml_and_send_dgt_manual(self):
        self.ensure_one()
        if (self.company_id.x_fae_mode in ('api-stag', 'api-prod')
            and (self.x_document_type and (not self.x_state_dgt or self.x_state_dgt == 'ENV') )) :
            ...
            try:
                ...
                if not inv.x_xml_comprobante or inv.x_state_dgt == 'ENV':
                    ...
                    for inv_line in inv.lines:
                        ...
                        if inv_line.product_id and not (inv_line.product_id.x_cabys_code_id and inv_line.product_id.x_cabys_code_id.code):
                            inv._message_post(subject='Error',
                ...
                if inv.x_state_dgt == '1':
                    response_status = 400
                    response_text = 'ya fue recibido anteriormente'
                else:
